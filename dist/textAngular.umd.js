...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taToolFunctions', '$sanitize', 'taOptions', function(taRegisterTool, $window, taTranslations, taSelection, taToolFunctions, $sanitize, taOptions){
  ...
  taRegisterTool('fontSize', {
      display: "<span class='bar-btn-dropdown dropdown' style='padding:0px !important;'>" +
              "<button class='btn btn-default dropdown-toggle' type='button' ng-disabled='showHtml()' style='padding-top: 4px' data-toggle='dropdown'><i class='fa fa-text-height'></i><i class='fa fa-caret-down'></i></button>" +
              "<ul class='dropdown-menu'><li ng-repeat='o in options' style='padding:0px !important;'><button class='btn btn-default checked-dropdown' style='font-size: {{o.css}}; width: 100%' type='button' ng-click='action($event, o.value)'><i ng-if='o.active' class='fa fa-check'></i> {{o.name}}</button></li></ul>" +
              "</span>",
      ...
      options: [
          { name: 'Extra-extra small', css: 'xx-small', value: 1 },
          { name: 'Extra small', css: 'x-small', value: 2 },
          { name: 'Small', css: 'small', value: 3 },
          { name: 'Medium', css: 'medium', value: 4 },
          { name: 'Large', css: 'large', value: 5 },
          { name: 'Extra large', css: 'x-large', value: 6 }
      ]
  });
  ...
  taRegisterTool('fontColor', {
      display: "<div spectrum-colorpicker ng-model='color' on-change='!!color && action(color)' format='\"hex\"' options='options' style='padding:0 !important'></div>",
      action: function (color) {
          var me = this;
          if (!this.$editor().wrapSelection) {
              setTimeout(function () {
...
var dropFired = false;
var textAngular = angular.module("textAngular", ['ngSanitize', 'textAngularSetup', 'textAngular.factories', 'textAngular.DOM', 'textAngular.validators', 'textAngular.taBind', 'angularSpectrumColorpicker']); //This makes ngSanitize required
